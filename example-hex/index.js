...
Diagram('parts', {
    data: {
        corner: hexCenter(new Hex(-2/3,1/3,1/3)).transform,
        side: hexCenter(new Hex(0,1/2,-1/2)).transform,
    },
});
    ...
    computed: {
        ...
        cornerMarkers() {
            return [0, 60, 120, 180, 240, 300].map(angle => ({
                angle: Math.round(angle - this.$root.rotation),
                transform: `rotate(${angle}) translate(100,0) rotate(${-angle})`,
            }));
        },
        cornerLabels() {
            return [0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330].map(angle => ({
                angle: Math.round(angle),
                transform: `rotate(${angle}) translate(112,0) rotate(${-angle})`,
            }));
        },
        dashedPath() {
    ...
    components: {
        'a-measurement': {
            ...
            template: `<g class="measurement">
               ...
               <text dy="0.4em" :transform="\`translate(\${midpoint.x.toFixed(0)},\${midpoint.y.toFixed(0)}) rotate(\${rotation}) translate(0,8)\`">{{label}}</text>
            </g>
            `,
            computed: {
                rotation() {
...
Diagram('conversions-offset', {
    ...
    computed: {
        ...
        output_constructor() {
            return this.output === 'axial'? "Axial(q, r)" : "Cube(q, r, -q-r)";
        },
    },
    methods: {
        className(hex) {
...
['rings', 'spiral'].forEach(id => Diagram(id, {
    ...
    computed: {
        ...
        ring() {
            let results = [];
            for (let radius = (id === 'spiral')? 1 : this.radius; radius <= this.radius; radius++) {
